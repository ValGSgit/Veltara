...
const panelTitle = computed(() => {
  if (props.panel === 'profile') return 'Profile';
  if (props.panel === 'social') return 'Social Feed';
  if (props.panel === 'store') return 'Marketplace';
  return 'Settings';
});
const panelSubtitle = computed(() => {
  if (props.panel === 'profile') return 'Identity, progress, and membership';
  if (props.panel === 'social') return 'Planet-wide moments and updates';
  if (props.panel === 'store') return 'Cosmetics, items, and credits';
  return 'Graphics, controls, and account';
});
...
const storeTypes = ['all', 'cosmetic', 'marker', 'frame', 'emote'];
...
const canSubmitPost = computed(() => postContent.value.trim().length > 0 && !loadingFeed.value);
...
<template>
  <Teleport to="body">
    <div class="fixed inset-0 z-40 pointer-events-none">
      <div class="absolute inset-0 bg-black/45 backdrop-blur-[2px] pointer-events-auto" @click="close"></div>
      <div class="fixed inset-y-0 w-[min(24rem,100vw)] overflow-hidden border-veltara-border bg-veltara-panel shadow-2xl transition-transform duration-300 pointer-events-auto"
        ...
        <div class="flex items-center justify-between border-b border-veltara-border p-4">
          <div>
            <h2 class="font-semibold text-white">{{ panelTitle }}</h2>
            <p class="mt-0.5 text-[11px] text-veltara-muted">{{ panelSubtitle }}</p>
          </div>
        ...
        <div v-if="panel === 'profile'" class="flex h-[calc(100%-65px)] flex-col overflow-y-auto gap-3 p-4">
          <PanelCard>
          ...
          <PanelCard>
            ...
            <textarea v-model="bio" rows="3" maxlength="200" class="input-field w-full resize-none text-xs" placeholder="Tell the world about yourself..." />
          ...
          <PanelCard v-if="user?.plan_tier === 'free'" class="border border-veltara-accent/30 bg-veltara-accent/10">
        ...
        <div v-else-if="panel === 'social'" class="flex h-[calc(100%-65px)] flex-col overflow-hidden">
          <PanelCard class="m-3 mb-2" compact>
            ...
            <div class="mt-2 flex items-center justify-end">
              <button class="btn-primary text-xs disabled:cursor-not-allowed disabled:opacity-60" :disabled="!canSubmitPost" @click="submitPost">Post</button>
            </div>
          </PanelCard>

          <div class="flex-1 overflow-y-auto px-3 pb-3">
            <div v-if="!feed.length && !loadingFeed" class="p-4 text-center text-xs text-veltara-muted">No posts yet. Be the first!</div>
            <article v-for="post in feed" :key="post.id" class="mb-2 rounded-xl border border-veltara-border p-3 transition-colors hover:bg-veltara-bg/40">
        ...
        <div v-else-if="panel === 'store'" class="flex h-[calc(100%-65px)] flex-col overflow-hidden">
          <div class="flex items-center gap-3 border-b border-veltara-border p-3">
            ...
            <div class="ml-auto flex gap-1 flex-wrap justify-end">
              <PillButton
                v-for="type in storeTypes"
                :key="type"
                class="store-filter capitalize"
                :active="storeType === type"
                @click="storeType = type; loadStoreItems()"
              >
                {{ type }}
              </PillButton>
            </div>
          ...
          <PanelCard class="m-3 mt-0" compact>
        ...
        <div v-else class="flex h-[calc(100%-65px)] flex-col overflow-y-auto gap-3 p-4">
          <PanelCard>
